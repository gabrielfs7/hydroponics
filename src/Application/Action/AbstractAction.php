...
abstract class AbstractAction
{
    ...
    abstract protected function process(RequestInterface $request, ResponseInterface $response, array $args): ResponseDtoInterface;

    protected function post(ResourceCreatorInterface $resourceCreator, RequestInterface $request): ResponseDtoInterface
    {
        return $resourceCreator->create((string) $request->getBody());
    }

    protected function patch(ResourceUpdaterInterface $resourceUpdater, RequestInterface $request): ResponseDtoInterface
    {
        return $resourceUpdater->update((string) $request->getBody(), $this->getRequestParameter($request, 'id'));
    }

    protected function delete(ResourceDeleterInterface $resourceDeleter, RequestInterface $request): ResponseDtoInterface
    {
        return $resourceDeleter->delete($this->getRequestParameter($request, 'id'));
    }

    protected function get(ResourceSearcherInterface $resourceSearcher, RequestInterface $request): ResponseDtoInterface
    {
        return $resourceSearcher->searchById($this->getRequestParameter($request, 'id'));
    }

    protected function getAll(ResourceSearcherInterface $resourceSearcher, RequestInterface $request): ResponseDtoInterface
    {
        return $resourceSearcher->search($this->getRequestParameters($request));
    }

    private function getRequestParameter(RequestInterface $request, string $parameter): string
    {
        /** @var Request $request */
